...
use crate::thumbnail::{ALLOWED_THUMB_SIZES, get_thumbnail};
                        ...
                        if let Ok(img) = image::open(&thumb_path) {
                            ...
                            if !app.thumb_texture_cache.contains_key(&cache_key) {
                                if app.thumb_texture_cache.len() >= 256 {
                                    if let Some(oldest) = app.thumb_cache_order.pop_front() {
                                        app.thumb_texture_cache.remove(&oldest);
                                    }
                                }
                                let texture = ui.ctx().load_texture(
                                    format!("thumb_{}_{}", current_meta_ref.filename, closest_size),
                                    color_image,
                                    egui::TextureOptions::default(),
                                );
                                app.thumb_texture_cache.insert(cache_key.clone(), texture);
                                app.thumb_cache_order.push_back(cache_key.clone());
                            }
                            if let Some(texture) = app.thumb_texture_cache.get(&cache_key) {
                                let display_width = app.grid_image_size;
                                let display_height = app.grid_image_size * 3.0_f32 / 4.0_f32;
                                ...
                                if image_response.clicked() {
                                    action = Some(ImageGridAction::ImageClicked(
                                ...
                                let ideal_text_x =
                                    top_bar_rect.center().x - text_galley.size().x / 2.0_f32;
                                let actual_text_x = ideal_text_x.max(icon_right_boundary);
                                let max_text_x =
                                    top_bar_rect.max.x - text_padding - text_galley.size().x;
