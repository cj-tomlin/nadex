use egui::{Ui, Sense, Vec2};
use crate::NadexApp;
use crate::persistence::{ImageMeta, NadeType};
use crate::thumbnail::{get_thumbnail, ALLOWED_THUMB_SIZES};
...
pub fn show_image_grid(
    app: &mut NadexApp, 
    ui: &mut Ui,
) -> Option<ImageGridAction> {
    let mut action: Option<ImageGridAction> = None;

    // Display image grid for app.current_map
    let data_dir_clone = app.data_dir.clone();
    let map_name_clone = app.current_map.clone();

    let images_for_current_map: Vec<ImageMeta> = app
        .image_manifest
        .images
        .get(&map_name_clone)
        .map_or_else(Vec::new, |v| v.clone());

    // Filter based on selected nade type (if any)
    let filtered_images: Vec<&ImageMeta> = images_for_current_map
        .iter()
        .filter(|meta| {
    ...
    egui::ScrollArea::vertical().show_viewport(ui, |ui, viewport| {
        ...
        let last_visible_row =
            ((offset_y + viewport.height()) / row_height).ceil() as usize + 1;
        ...
        grid.show(ui, |ui| {
            for (i, meta) in filtered_images.iter().enumerate() {
                ...
                if this_row < first_visible_row || this_row > last_visible_row {
                    let display_width = app.grid_image_size;
                    let display_height = app.grid_image_size * 3.0_f32 / 4.0_f32;
                    let (rect_alloc, _) = ui.allocate_exact_size(Vec2::new(display_width, display_height), Sense::hover());
                    ...
                    let img_path = data_dir_clone
                        ...
                        .join(&current_meta_ref.filename);
                    let thumb_dir =
                        data_dir_clone.join(&map_name_clone).join(".thumbnails");
                    let requested_size = app.grid_image_size as u32;
                    let &closest_size = ALLOWED_THUMB_SIZES
                        .iter()
                        .min_by_key(|&&s| (s as i32 - requested_size as i32).abs())
                        .unwrap_or(&480);
                    let cache_key = (current_meta_ref.filename.clone(), closest_size);
                        ...
                        get_thumbnail(&img_path, &thumb_dir, closest_size)
                    {
                        if let Ok(img) = image::open(&thumb_path) {
                            ...
                            if !app.thumb_texture_cache.contains_key(&cache_key) {
                                if app.thumb_texture_cache.len() >= 256 {
                                    if let Some(oldest) =
                                        app.thumb_cache_order.pop_front()
                                    {
                                        app.thumb_texture_cache.remove(&oldest);
                                    }
                                }
                                let texture = ui.ctx().load_texture(
                                    format!(
                                        "thumb_{}_{}",
                                        current_meta_ref.filename, closest_size
                                    ),
                                    color_image,
                                    egui::TextureOptions::default(),
                                );
                                app.thumb_texture_cache
                                    .insert(cache_key.clone(), texture);
                                app.thumb_cache_order.push_back(cache_key.clone());
                            }
                            if let Some(texture) =
                                app.thumb_texture_cache.get(&cache_key)
                            {
                                let display_width = app.grid_image_size;
                                let display_height =
                                    app.grid_image_size * 3.0_f32 / 4.0_f32;
                                let image_widget =
                                    egui::Image::new(egui::load::SizedTexture::new(
                                        texture.id(),
                                        texture.size_vec2(),
                                    ))
                                    .corner_radius(egui::CornerRadius::same(4_u8));
                                ...
                                let icon_center_y =
                                    top_bar_rect.min.y + bar_height / 2.0_f32;
                                let icon_center_x =
                                ...
                                let position_text_str =
                                    if current_meta_ref.position.is_empty() {
